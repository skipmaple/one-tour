...
export default function ConstitutionFullText({ constitution }) {
  ...
  return (
    <div style={DOC.wrapper}>
      ...
      <p style={DOC.body}>一个「日」包含多个有规律的「行」，遵循人类作息和当地节奏。伊犁比北京偏西约 30 个经度，手机时间减 2 小时 = 体感时间。</p>
      ...
      <p style={DOC.body}>路不是管道。一等公民路（独库、伊昭）行驶过程本身就是目的；二等公民路（果子沟大桥段）沿途有景观但不需全程慢行；普通路段仅追求效率。</p>

      <p style={DOC.article}>第七条　食（二等或三等公民）</p>
      <p style={DOC.body}>默认三等公民。满足必吃清单 / 当地特色 / 路线重合条件时可升级为二等公民，但整程不超过<span style={DOC.red}> {t2Food} 家</span>。</p>

      <p style={DOC.article}>第八条　住（三等公民）</p>
      <p style={DOC.body}>围绕当日最后一个景点就近安排。底线：女生可接受的卫生条件、独立卫浴、手机可充电。毡房/蒙古包体验整程最多 1 晚。</p>

      <p style={DOC.article}>第九条　油（基础设施）</p>
      <p style={DOC.body}>铁律：逢镇必加，半箱即补。油量低于 1/4 且前方 100km 无站时升级为一等公民，整程此类升级不超过<span style={DOC.red}> {fuelEmg} 次</span>。</p>
      ...
      <p style={DOC.body}>单日总驾驶时间<span style={DOC.red}> ≤ {driveH} 小时</span>（含所有路段），其中山路段<span style={DOC.red}> ≤ {mtH} 小时</span>。超出视为违宪，必须拆分或砍行程。</p>

      <p style={DOC.article}>第十一条　每日一等公民数量上限</p>
      <p style={DOC.body}>单日安排的一等公民（景 + 一等公民路）总数<span style={DOC.red}> ≤ {t1} 个</span>。每个至少需要 1.5-3 小时体验。</p>

      <p style={DOC.article}>第十二条　机动时间预留</p>
      <p style={DOC.body}>每日预留<span style={DOC.red}> ≥ {bufMin} 分钟</span>未分配时间；整程预留<span style={DOC.red}> ≥ {bufDays} 个完整机动日</span>。</p>
      ...
      <p style={DOC.body}>避开大流量方向；最期待的景点放在体力天气最好的日子；独库建议南→北走。</p>
