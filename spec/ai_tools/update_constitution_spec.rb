...
RSpec.describe AITools::UpdateConstitution do
  it "merges allowed keys into tour.constitution" do
    tour = create(:tour)
    described_class.new(tour: tour).execute(patch: { "max_mountain_road_minutes" => 300 })
    expect(tour.reload.constitution["max_mountain_road_minutes"]).to eq(300)
  end

  it "ignores unknown keys" do
    tour = create(:tour)
    described_class.new(tour: tour).execute(patch: { "bogus_key" => 999 })
  ...
  it "bails with tour_context_missing when constructed without a tour" do
    result = described_class.new.execute(patch: { "max_tier_one_per_day" => 4 })
