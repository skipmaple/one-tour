...
RSpec.describe "PATCH /profile", type: :request do
  ...
  it "redirects to /login when not logged in" do
    patch "/profile", params: { name: "Changed" }
  ...
  describe "when logged in" do
    ...
    it "returns errors for an invalid name" do
      patch "/profile", params: { name: "bad name" }, headers: { "X-Inertia" => "true" }
      expect(user.reload.name).to eq("Original")
    end

    it "attaches an uploaded avatar" do
      patch "/profile", params: { avatar: valid_png }
    ...
    it "rejects an oversized file" do
      ...
      oversize.rewind
      patch "/profile", params: { avatar: Rack::Test::UploadedFile.new(oversize.path, "image/png") }
      user.reload
