...
>
> 生产环境架构(SWAS HK + OSS HK proxy mode + 切换日陷阱沉淀)详见 [swas-cutover.md](swas-cutover.md)。
...
| **上传链路** | 客户端压缩(`browser-image-compression`)+ 自写 `xhrRequest` helper(retry / progress / abort / Sentry) |
...
| 上传 | `xhrRequest` helper(`app/javascript/lib/xhr-request.js`)+ 客户端压缩,Active Storage **proxy mode**(见 [swas-cutover.md](swas-cutover.md))| 不引入 tus / tusd · 不用 Direct Upload |
...
> **v1.3 修订**:Direct Upload 路径在切换日 OSS SigV4 兼容性事故后被砍(见 [swas-cutover.md](swas-cutover.md) "已知陷阱 1"),改走客户端压缩 + 自写 `xhrRequest` helper + Active Storage proxy mode。

- [x] 客户端图片压缩:`browser-image-compression`,JPEG/HEIC → WebP,长边 ≤ 2048px,GIF / <500KB 跳过
- [x] `xhrRequest` helper:FormData/JSON 两栖,指数退避 3 次(1s / 2s),仅 408/429/500/502/503/504 + 网络错误 retry,419/422 立即失败,AbortSignal,CSRF 自动注入,Sentry 终态上报(`tag: endpoint/final_status/effective_type`)
- [x] 进度 UI:Mantine `Progress` 单一组件,无文字 —— ActivityGalleryTab 横扫整 batch,AddExpenseDialog 字节级聚合(并发模型保留)
- [x] `useGalleryUploader` hook:从 `ActivityGalleryTab` 抽出,`unmountedRef` 守护 unmount-mid-flight 的副作用
