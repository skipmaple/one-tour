...
- **Deploying staging** — Never `kamal -d staging ...`. `-d` is `deploy.yml` deep-merged with `deploy.<dest>.yml`,**不是替换**;prod 的 `servers.web: [43.103.50.22]` 不会被 staging 的 `[45.63.23.136]` 替换,两台都进 resolved hosts。下一步 `app boot` 会以 staging tag + `RAILS_ENV=staging` 在 prod host 起容器,接管流量,prod 当场炸。2026-04-29 真踩过,build 阶段 Ctrl+C 才没爆。永远 `scripts/kamal-staging.sh ...` —— 强制 `-c config/staging/deploy.yml`(单文件,不 merge)+ pre-flight grep prod IP + 把 `.kamal/staging/secrets` 拷到 `.kamal/secrets`(kamal source 顺序里 secrets-common 先 / staging 后,共享 key 自然被 staging 值覆盖,不动 secrets-common)。**generalize**:任何"我以为是隔离"的运维命令,destructive 步骤前先 dry-run resolved config(`kamal config -c ...`)grep host 列表。
