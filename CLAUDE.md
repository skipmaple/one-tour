...
- **Editing expense / settlement math** — `paid_cents` deliberately excludes `individual` (各付各) expenses; they're self-contained (money the payer spent on themselves) and stay out of the settlement ledger — including them once produced a phantom receivable. `my_spend_cents` (`owed` + my own individual expenses) is the "what I actually bore" number the budget card reads. Full four-field model is explained across `app/models/expense/summarize.rb` — class header covers the ledger stance, `current_user_balance` method covers `my_spend_cents` — read both before touching it.
- **Writing a new Inertia mutation controller action** — Branch on `inertia_request?`: Inertia callers get `redirect_to path, alert: msg` (error) / `notice: msg` (success) / bare `redirect_to path` (success, no flash); fetch and specs get `render json:`. Returning JSON to Inertia pops an "invalid response" modal in the browser — 4 mutation endpoints hit this before we noticed. Canonical shape: `ExpensesController#update`, `SettlementsController#create`; full rationale on `ApplicationController#inertia_request?`.
- **Deploying staging** — Never `kamal -d staging ...`. `-d` deep-merges `deploy.<dest>.yml` into `deploy.yml`, **not replaces it**, so prod's `servers.web: [43.103.50.22]` survives even when staging declares `[45.63.23.136]`; both hosts end up in resolved hosts. Next step `app boot` will pull the staging tag onto prod with `RAILS_ENV=staging`, take over traffic, and torch prod. Caught 2026-04-29 mid-build by Ctrl+C. Always go through `scripts/kamal-staging.sh ...` instead — it forces `-c config/staging/deploy.yml` (single file, no merge), pre-flights `kamal config | grep <prod-IP>` and exits 1 on match, and copies `.kamal/staging/secrets` to `.kamal/secrets` so kamal's source order (secrets-common first, staging-secrets second) lets staging values override shared keys without touching secrets-common. **Generalize**: for any "isolated environment" ops command, dry-run resolved config (`kamal config -c ...`) and grep the host list before any destructive step.
